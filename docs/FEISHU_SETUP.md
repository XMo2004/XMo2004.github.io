...
| 专栏序号 | 数字 | 条件必填 | 专栏内的阅读顺序；必须是正整数，并与「专栏」同时填写。 |
| 发布日期 | 日期 | 是 | 文章公开日期。 |
| 状态 | 单选 | 是 | 只使用「草稿」「已发布」「已下线」。同步仅发布「已发布」。 |
| 精选 | 复选框 | 否 | 勾选后进入首页精选区；空值按否处理。 |
| 封面 | 附件 | 否 | 可上传一张封面；留空时站点使用排版封面。 |

`Slug` 不能与手写文章或另一条飞书记录重复。准备发布时，必须先选择「分类」；如果文章属于专栏，「专栏」和「专栏序号」要成对填写，不属于专栏时则两者都留空。把记录改为「草稿」或「已下线」后，下次同步会从飞书生成目录中移除对应文章；手写文章不会受影响。
...
错误信息会带 record ID 和字段名。`Slug` 只能使用小写字母、数字与连字符；发布日期必须是有效日期；「分类」是发布必填的单选字段；「专栏」与「专栏序号」必须同时填写或同时留空，序号只接受正整数；状态只能是三个约定选项。字段名称前后的空格也会造成读取失败。
